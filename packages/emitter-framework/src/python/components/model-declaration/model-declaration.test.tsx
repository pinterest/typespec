...
describe("Python Model Declaration - Non-Record", () => {
  ...
  it("converts a model with a field that can be null", async () => {
    ...
    expect(getOutput(runner.program, [<ModelDeclaration type={result.Person} />])).toRenderTo(`
  ...
  //       <ModelDeclaration type={result.Address} />,
  //       <ModelDeclaration type={result.EuropeAddress} />,
  //     ]),
  //   ).toRenderTo(`
  //         from dataclasses import dataclass
  ...
  it("converts a model with model inheritance through extends", async () => {
    ...
    expect(
      getOutput(runner.program, [
        <ModelDeclaration type={result.Identified} />,
        <ModelDeclaration type={result.Admin} />,
      ]),
    ).toRenderTo(`
          from dataclasses import dataclass
  ...
  it("converts a model with model inheritance through spreads", async () => {
    ...
    expect(
      getOutput(runner.program, [
        <ModelDeclaration type={result.Animal} />,
        <ModelDeclaration type={result.Dog} />,
      ]),
    ).toRenderTo(`
      from dataclasses import dataclass
  ...
  it("declares a model with multi line docs, explicit docs passed", async () => {
    ...
    expect(
      getOutput(runner.program, [
        <ModelDeclaration type={result.Foo} />,
      ]),
    ).toRenderTo(`
      from dataclasses import dataclass
